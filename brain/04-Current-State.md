...
- Stage 6 has ADR-0006 and ADR-0007: `@tatu/shared` defines readonly model suitability metadata plus a separate `ProviderStateStore`; `InMemoryProviderStateStore` validates and tracks health, quota, rate limits, latency, and typed failures with frozen defensive snapshots. The implementation is process-local and observation-only: it does not call providers, persist state, hold credentials, or route executions. The full suite passes with 40 tests; Model Router, Quota/Provider Router, eligible-route selection, BYOK, and fallback orchestration remain future Stage 6 work. The deterministic RSS result remains a non-model fallback.
