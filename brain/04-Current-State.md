...
- Stage 5 research foundations are partially complete: `@tatu/research` defines database-independent feed/synthesizer ports and a credential-free public HTTPS RSS adapter with cancellation, canonical URL/title deduplication, relevance-first ranking, source validation, bounded public fetching, private-destination rejection, structured facts, and an explicit empty inference list. The worker persists valid results and emits `research_failed` for missing configuration or unsafe/unavailable/insufficient feeds; it never treats an unconfigured feed list as success. `GET /api/executions/:id/briefing` returns validated saved results. `npm run ci` passed with 25 tests. No default feed, model route, delivery channel, or full daily acceptance run has been selected yet, so Stage 5 remains partial.
